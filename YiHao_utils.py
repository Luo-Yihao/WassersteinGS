...
## Wasserstein of two gaussians of yihao origin
...
#         assert torch.isfinite(scale1).all(), "scale1 包含 Inf 或 NaN"
#         assert (scale1 > 0).all(), "scale1 包含非正值"
#         if scale2 is not None:
#             assert torch.isfinite(scale2).all(), "scale2 包含 Inf 或 NaN"
#             assert (scale2 > 0).all(), "scale2 包含非正值"
...
#             # 如果cov2为None，则根据 R2，S2 计算得到 cov2，不为 None 时忽略 R2，S2
...
#         assert torch.allclose(E, E.transpose(1, 2), atol=1e-6), "E 矩阵不对称"


#         E_eign = torch.linalg.eigvalsh(E)
#         assert torch.isfinite(E_eign).all(), "E 的特征值包含 Inf 或 NaN"
#         assert (E_eign >= 0).all(), "E 的特征值包含负值，可能导致复数平方根"
...
#         assert torch.isfinite(CovWasserstein).all(), "CovWasserstein 包含 Inf 或 NaN"
#         assert (CovWasserstein >= 0).all(), "CovWasserstein 包含负值"
...
#     # 进行特征值分解
#     L, Q = torch.linalg.eigh(A)
#     L = torch.clamp(L, min=eps)  # 确保特征值为正
#     sqrt_L = torch.sqrt(L)
#     sqrt_A = Q @ torch.diag_embed(sqrt_L) @ Q.transpose(-1, -2)
#     return sqrt_A

'''
Junli的实现
'''
# def matrix_sqrt_inv(A, eps=1e-6):
#     """
#     计算对称正定矩阵的逆平方根
#     """
#     # 确保 A 是对称的
...
#     sqrt_inv_A = Q @ torch.diag_embed(inv_sqrt_L) @ Q.transpose(-1, -2)
#     return sqrt_inv_A
...
if __name__ == "__main__":
    ...
    '''
    测试 yihao 方法和 4dgs 源码中的方法计算协方差矩阵是否一致
    '''
    # test_cov(R,S)
