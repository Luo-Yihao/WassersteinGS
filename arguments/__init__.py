...
class PipelineParams(ParamGroup):
    def __init__(self, parser):
        ...
        self.compute_cov3D_python = True # False
        if self.compute_cov3D_python:
            debug_print("add compute_cov3D_python ，在 python 中计算 3D 协方差，而不是在 cuda 中带入 R 和 S 计算")
        self.kalman_filter = True # True 为加上卡尔曼滤波预测
        if self.kalman_filter:
            debug_print("add kalman_filter")
        else:
            debug_print("NOT add kalman_filter")
        # debug_print("add compute_cov3D_python ，在 python 中计算 3D 协方差，而不是在 cuda 中带入 R 和 S 计算")
        self.debug = False
...
class OptimizationParams(ParamGroup):
    def __init__(self, parser):
        ...
        self.lambda_dssim = 0
        self.lambda_lpips = 0
        self.lambda_wasserstein = 0 #0.00001 # 大于0时，使用Wasserstein距离
        if self.lambda_wasserstein == 0:
            debug_print("NOT add lambda_wasserstein")
        else:
            debug_print("add lambda_wasserstein")
        self.wasserstein_from_iter = 9000 # 2000 # must be larger than 2
        ...
        self.batch_size= 3 # 1 ，# 3 用于 kalman 滤波
        if self.batch_size == 1:
            debug_print("单帧训练！！！batch_size: " + str(self.batch_size))
        else:
            debug_print("为了适应卡尔曼滤波！！！batch_size: " + str(self.batch_size))
